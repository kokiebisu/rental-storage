...
func (s *BookingService) CreateBooking(id string, userId string, spaceId string, imageUrls []string, createdAt string, updatedAt string) (string, *customerror.CustomError) {
	bookingEntity, err := booking.New(id, userId, spaceId, imageUrls, createdAt, updatedAt)
	if err != nil {
		return "", err
	}
