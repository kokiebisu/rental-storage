...
// CreateBooking provides a mock function with given fields: id, userId, spaceId, imageUrls, createdAt, updatedAt
func (_m *BookingService) CreateBooking(id string, userId string, spaceId string, imageUrls []string, createdAt string, updatedAt string) (string, *errors.CustomError) {
	ret := _m.Called(id, userId, spaceId, imageUrls, createdAt, updatedAt)

	var r0 string
	if rf, ok := ret.Get(0).(func(string, string, string, []string, string, string) string); ok {
		r0 = rf(id, userId, spaceId, imageUrls, createdAt, updatedAt)
	} else {
		r0 = ret.Get(0).(string)
	}

	var r1 *errors.CustomError
	if rf, ok := ret.Get(1).(func(string, string, string, []string, string, string) *errors.CustomError); ok {
		r1 = rf(id, userId, spaceId, imageUrls, createdAt, updatedAt)
	} else {
		if ret.Get(1) != nil {
