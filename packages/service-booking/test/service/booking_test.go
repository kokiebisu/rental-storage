...
func setupTest(t *testing.T) (string, *customerror.CustomError) {
	...
	token, err := data.BookingService.CreateBooking(data.MockUId, data.MockUserId, data.MockSpaceId, data.MockBookingEntity.ImageUrls, data.MockDateString, data.MockDateString)
	return token, err
}
